...
/** The process environment boundary. `get` delegates to the JVM on every call,
  * so Windows' case-insensitive variable names keep working instead of being
  * lost in a copied Scala map. */
