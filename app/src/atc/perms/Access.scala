...
object Access:
  def parse(s: String): Access = s.trim.toLowerCase(Locale.ROOT) match
...
/** The effective permission of a path. `classified` means the content (and, for directories, the
  * structure) may only be observed through `Classified` values; `locked` means the configuration
  * forbids widening the access through a user prompt. */
final case class Perm(access: Access, classified: Boolean, locked: Boolean = false):
