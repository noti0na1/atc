...
import atc.config.{Config, ModelSpec, Tokens}
...
final class ChatGPTModel(spec: ModelSpec, auth: ChatGPTAuth) extends OpenAIResponsesModel(spec):
  ...
  override protected def send(params: ResponseCreateParams): Response =
    ...
    scala.util.Using.resource(client.responses().createStreaming(complete))(_.stream().forEach(acc.accumulate(_)))
    acc.response()
  ...
  override private[llm] def listModels(): List[ModelSpec] =
    ...
    val text = scala.util.Using.resource(call.execute()) { response =>
      val body = Option(response.body).fold("")(_.string())
      if !response.isSuccessful then throw java.io.IOException(s"HTTP ${response.code}: ${body.take(300)}")
      body
    }
      ...
      .flatMap { m =>
        str(m, "slug").map { id =>
          val efforts = m.obj.get("supported_reasoning_levels").flatMap(_.arrOpt).toList.flatten
            ...
            .map(_.toLowerCase(java.util.Locale.ROOT)).filter(Config.ReasoningEfforts.contains)
          val reasoning = str(m, "default_reasoning_level").map(_.toLowerCase(java.util.Locale.ROOT))
