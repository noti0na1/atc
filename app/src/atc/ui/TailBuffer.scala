...
/** Bounded output buffer with incremental line counts. `tail(n)` scans backward
  ...
  * The buffer grows to half as much again before it is cut back to `cap`, so a
  * stream of small appends does not move the whole text on each one. */
private[atc] final class TailBuffer(cap: Int):
  ...
  def append(text: String): Unit =
    ...
    sb.append(text)
    if sb.length > cap + cap / 2 then
      val nl = sb.indexOf("\n", sb.length - cap)
      sb.delete(0, if nl >= 0 then nl + 1 else sb.length - cap)
  /** Everything retained: the whole text while under the cap, then at least the lines of its last `cap` characters. */
